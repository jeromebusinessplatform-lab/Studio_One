    ...
    <div className="bg-[#f3f4f6] min-h-full pb-10">
      ...
      <div className="p-3 space-y-3">
        <div className="bg-white rounded-2xl border border-neutral-200 p-4 shadow-sm text-center">
          <div className="w-20 h-20 rounded-full bg-neutral-200 mx-auto mb-3 flex items-center justify-center overflow-hidden">{avatarUrl ? <img src={avatarUrl} alt="Telegram profile avatar" className="w-full h-full object-cover" referrerPolicy="no-referrer" /> : <User size={40} className="text-neutral-500" />}</div>
          <div className="text-lg font-bold">{customerData.telegramDisplayName}</div><div className="text-neutral-500 text-sm mb-4">{customerData.telegramUsername ? `@${customerData.telegramUsername}` : "Telegram Customer"}</div>
          <input type="file" ref={fileInputRef} onChange={handleFileChange} accept="image/*" className="hidden" />
          <button onClick={() => fileInputRef.current?.click()} disabled={isUpdating} className="w-full text-xs p-2 bg-black text-white rounded font-bold flex items-center justify-center gap-2"><Upload size={14}/>{isUpdating ? "Uploading..." : "Upload Avatar"}</button>
          <div className="grid grid-cols-2 gap-4 mt-6 text-xs text-left"><div className="border p-2 rounded"><div className="text-neutral-500">TELEGRAM UID</div><div>{customerData.telegramUserId}</div></div><div className="border p-2 rounded"><div className="text-neutral-500">PRIME MEMBER ID</div><div>{customerData.primeMemberId}</div></div></div>
        </div>
