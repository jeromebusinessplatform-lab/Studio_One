...
export default function CheckoutPage() {
  ...
  const itemsKey = checkoutItems.map((i) => `${i.productId}:${i.quantity}`).join("|");
  ...
  const payloadKey = useMemo(() => `${itemsKey}::${courierId}::${distanceKm.toFixed(3)}::${deliveryPaymentOption}`, [itemsKey, courierId, distanceKm, deliveryPaymentOption]);

  useEffect(() => {
    let cancelled = false;
    if (!customer?.telegramUserId || !selectedCourier || !routeInfo || !checkoutItems.length) {
      setQuote(null);
      setQuoteError(null);
      return;
    }
  ...
  }, [customer?.telegramUserId, payloadKey, checkoutItems.length, routeInfo, selectedCourier, itemsKey]);

  const validateReceiver = () => {
    if (!customer?.telegramUserId) return toast.error("Open checkout from Telegram to continue.");
    if (receiver.trim().length < 2) return toast.error("Enter the receiver name.");
    if (!/^[0-9+()\-\s]{7,30}$/.test(phone.trim())) return toast.error("Enter a valid contact phone number.");
    if (!addressInput.trim() || !selectedLocation) return toast.error("Select a delivery address from the suggested addresses.");
    return true;
  };
  const validateDelivery = () => {
    if (!selectedCourier?.isAvailable) return toast.error("Select an available delivery provider.");
    if (!routeInfo || isCalculatingRoute) return toast.error("Wait for the delivery route to finish calculating.");
    if (!quote || quoteLoading) return toast.error(quoteError || "Secure checkout pricing is not ready yet.");
    return true;
  };
  const validatePayment = () => {
    if (paymentMethod === "DIRECT_TRANSFER" && !receiptUrl) return toast.error("Upload payment proof before submitting.");
    return true;
  };
...
function PrimaryButton({label,disabled=false}:{label:string;disabled?:boolean}){return <button type="submit" disabled={disabled} className="flex-[2] bg-black text-white py-3.5 rounded-xl flex items-center justify-center gap-2 disabled:opacity-40">{label}<ArrowRight size={17}/></button>}
