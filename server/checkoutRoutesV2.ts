...
const FALLBACK_COURIERS: Record<string, any> = {
  "courier-1": { id: "courier-1", name: "PRIME In-House Express", baseFare: 60, baseDistanceKm: 4, perKmCharge: 12, platformFeeEnabled: false, platformFee: 0, nightDifferentialEnabled: true, nightDifferentialFee: 30, surchargeEnabled: false, surchargeFee: 0, isAvailable: true },
  "courier-2": { id: "courier-2", name: "Lalamove 2-Wheel", baseFare: 70, baseDistanceKm: 3, perKmCharge: 15, platformFeeEnabled: true, platformFee: 10, nightDifferentialEnabled: true, nightDifferentialFee: 40, surchargeEnabled: true, surchargeFee: 20, isAvailable: true },
  "courier-3": { id: "courier-3", name: "GrabExpress Flash", baseFare: 80, baseDistanceKm: 5, perKmCharge: 18, platformFeeEnabled: true, platformFee: 15, nightDifferentialEnabled: false, nightDifferentialFee: 0, surchargeEnabled: false, surchargeFee: 0, isAvailable: true },
};
...
async function buildQuote(input: any, telegramId: string) {
  ...
  if ((hasCouponInput || hasReferralInput) && !couponValid) {
    // Quote requests should be able to render the validator error without blocking the page.
    promo = null;
  }
